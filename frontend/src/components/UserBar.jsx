function UserBar({ user, onLogout, compact = false }) {
  ...
  return (
    <div
      className={
        compact
          ? 'mb-5 flex min-w-0 flex-wrap items-center justify-between gap-3 rounded-3xl border border-white/10 bg-white/5 px-4 py-3 shadow-[0_24px_60px_rgba(0,0,0,0.28)] backdrop-blur-xl'
    ...
    >
      <div className="min-w-0">
        <strong className={compact ? 'block max-w-56 truncate text-sm font-semibold text-stone-50 sm:max-w-none' : 'block text-lg font-semibold text-stone-50'}>
          {user.name}
        </strong>
      </div>
